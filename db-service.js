(() => {
  ...
  function rowLedger(x){ const raw=clone(x); return {id:text(x.id), user_id:x.userId||x.user_id, account_type:x.accountType||x.account_type||"REAL", type:x.type||"WALLET", amount:num(x.amount), reference_id:text(x.referenceId||x.reference_id||x.id), note:text(x.note), balance_after:num(x.balanceAfter||x.balance_after), created_at:iso(x.createdAt||x.created_at), raw}; }
  function stateLedger(r){ const raw=r.raw&&typeof r.raw==="object"?r.raw:{}; return {...raw,id:r.id,userId:r.user_id,accountType:r.account_type||"REAL",type:r.type,amount:num(r.amount),referenceId:r.reference_id,note:r.note||raw.note||"",balanceAfter:num(r.balance_after),createdAt:r.created_at||raw.createdAt}; }
  ...
  async function writeSettings(settings){ assertReady(); pauseLocalLiveSync(); const row={id:"main",settings:{...clone(settings||App.state.settings||{}),databaseRuntimeVersion:"5.37",updatedBy:"admin"},updated_at:new Date().toISOString()}; const {error}=await client.from("app_settings").upsert(row,{onConflict:"id"}); if(error) throw error; return row; }

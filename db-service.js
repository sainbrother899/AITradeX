(() => {
  ...
  async function writeSettings(settings){ assertReady(); pauseLocalLiveSync(); const row={id:"main",settings:{...clone(settings||App.state.settings||{}),databaseRuntimeVersion:"6.9",updatedBy:"admin"},updated_at:new Date().toISOString()}; const {error}=await client.from("app_settings").upsert(row,{onConflict:"id"}); if(error) throw error; return row; }
